...
4. **Sanity check.** Open the **Certification Exam** record from the navigation bar or **Object Manager → Certification_Exam\_\_c → records**. Confirm `Published Question Count` is > 0.
...
### 3.3 Generation Jobs _(was: "waiting for event")_
...
| `Cert_Game_All_Admin` _(perm set group)_ | Anyone who needs both                 | Combines admin + player.                                                                                            |
