...
export default class GenerationJobConsole extends LightningElement {
    ...
    @track summary = { jobs: [], totalJobs: 0, runningJobs: 0, completedJobs: 0, failedJobs: 0, questionsGenerated: 0, tokenCostMtd: 0 };
    ...
    get hasJobs() { return this.summary && this.summary.jobs && this.summary.jobs.length > 0; }
    get hasLiveEvents() { return this.liveEvents && this.liveEvents.length > 0; }
    get tokenCostMtdDisplay() {
        const v = this.summary && this.summary.tokenCostMtd != null ? this.summary.tokenCostMtd : 0;
        return '$' + Number(v).toFixed(2);
    }

    get sourceOptions() { return this.exams || []; }
    get providerOptions() {
        return [
    ...
    get isStepTarget() { return this.step === STEP_TARGET; }
    get isStepMix() { return this.step === STEP_MIX; }
    get isStepNotes() { return this.step === STEP_NOTES; }
    get isStepConfirm() { return this.step === STEP_CONFIRM; }

    get step1Class() { return this.stepClass(STEP_TARGET); }
    get step2Class() { return this.stepClass(STEP_MIX); }
    get step3Class() { return this.stepClass(STEP_NOTES); }
    get step4Class() { return this.stepClass(STEP_CONFIRM); }
    ...
    get selectedDomainsLabel() {
        return (this.selectedDomains || []).join(', ') || 'All domains (no filter)';
    }
    get nextDisabled() {
        if (this.busy) return true;
        if (this.isStepTarget) return !this.selectedExamId || !this.questionCount || this.questionCount < 1;
        return false;
    }
    ...
    decorateSummary(s) {
        const jobs = (s.jobs || []).map((j) => {
            ...
            const statusClass = status === 'Completed' ? 'gjc-pill gjc-pill-completed'
                : status === 'Failed' ? 'gjc-pill gjc-pill-failed'
                : status === 'Running' ? 'gjc-pill gjc-pill-running'
                : 'gjc-pill gjc-pill-queued';
            const examLabel = j.examCode ? `${j.examCode} — ${j.examName || ''}` : (j.examName || '—');
            const createdDisplay = j.createdDate ? new Date(j.createdDate).toLocaleString() : '—';
            const costDisplay = j.tokenCostUsd ? '$' + Number(j.tokenCostUsd).toFixed(3) : '—';
            return { ...j, statusClass, examLabel, createdDisplay, costDisplay };
        });
        return { ...s, jobs };
    }
    ...
    async handleRerun(e) {
        ...
        try {
            ...
            this.toast('Re-queued', 'A fresh generation job has been queued.', 'success');
            this.handleRefresh();
        } catch (err) {
    ...
    handleCopyJson() {
        ...
        navigator.clipboard.writeText(this.jobDetail.outputJson)
            .then(() => this.toast('Copied', 'Output JSON copied to clipboard.', 'success'))
            .catch(() => this.toast('Copy failed', 'Clipboard not available in this browser.', 'warning'));
    }

    handleDownloadJson() {
        if (!this.jobDetail || !this.jobDetail.outputJson) return;
        const blob = new Blob([this.jobDetail.outputJson], { type: 'application/json' });
    ...
    handleViewQuestionsDeepLink() {
        window.open('/lightning/n/Question_Review?c__status=Needs Review', '_blank');
    }
    ...
    handleExamChange(e) {
        ...
        if (this.selectedExamId) {
            listExamDomains({ examId: this.selectedExamId })
                .then((rows) => { this.examDomains = rows; })
                .catch(() => { this.examDomains = []; });
        } else {
            this.examDomains = [];
        }
    }
    ...
    handleDifficultyChange(e) { this.difficulty = e.detail.value; }
    handleDomainsChange(e) { this.selectedDomains = e.detail.value || []; }
    handleNotesChange(e) { this.promptNotes = e.target.value; }

    handleNext() {
        if (this.isStepTarget) { this.step = STEP_MIX; return; }
        if (this.isStepMix) { this.step = STEP_NOTES; return; }
        if (this.isStepNotes) { this.step = STEP_CONFIRM; return; }
    }
    handleBack() {
        if (this.isStepConfirm) { this.step = STEP_NOTES; return; }
        if (this.isStepNotes) { this.step = STEP_MIX; return; }
        if (this.isStepMix) { this.step = STEP_TARGET; return; }
    }
