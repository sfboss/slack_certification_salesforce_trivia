...
export default class CertGameLeaderboard extends NavigationMixin(
    LightningElement
) {
    @api tenantId;
    @api defaultLimit = 25;
    ...
    get examOptions() {
        ...
        return base.concat(
            this.data.exams.map((e) => ({ label: e.name, value: e.id }))
        );
    }
        ...
        return [
            {
                ...
                value: this.fmtInt(s.activePlayers),
                sub: `of ${this.fmtInt(s.totalPlayers)} registered`,
                icon: "utility:groups",
                tone: "brand"
            },
            {
                key: "games",
                label: "Games played",
                value: this.fmtInt(s.totalGames),
                sub: `${this.fmtInt(s.totalAnswers)} answers`,
                icon: "utility:trophy",
                tone: "success"
            },
            {
                ...
                value: this.fmtInt(s.totalPoints),
                sub: s.windowLabel,
            ...
            },
            {
                key: "accuracy",
                label: "Avg accuracy",
                value: `${(s.avgAccuracy || 0).toFixed(1)}%`,
                sub: `Top streak: ${this.fmtInt(s.topStreak)}d`,
                icon: "utility:target",
                tone: "inverse"
            }
        ];
    ...
    get podium() {
        ...
        if (byRank[2])
            ordered.push({
                ...byRank[2],
                podiumClass: "podium-card podium-silver",
                label: "2nd",
                emoji: "🥈"
            });
        if (byRank[1])
            ordered.push({
                ...byRank[1],
                podiumClass: "podium-card podium-gold center",
                label: "1st",
                emoji: "🥇"
            });
        if (byRank[3])
            ordered.push({
                ...byRank[3],
                podiumClass: "podium-card podium-bronze",
                label: "3rd",
                emoji: "🥉"
            });
    ...
    get rows() {
        ...
        return list.map((r, i) => ({
            ...
            rowClass:
                "lb-row" +
                (i % 2 === 0 ? " lb-row-even" : "") +
                (r.medal ? ` lb-row-${r.medal}` : ""),
            medalBadgeClass: r.medal
                ? `medal-badge medal-${r.medal}`
                : "medal-badge medal-none",
            medalIcon:
                r.medal === "gold"
                    ? "🥇"
                    : r.medal === "silver"
            ...
            achievementsLabel: r.achievementCount
                ? `🏆 ${r.achievementCount}`
                : "—"
        }));
    }
    ...
    get rankSortLabel() {
        return `#${this.sortIcon("rank")}`;
    }
    get nameSortLabel() {
        return `Player${this.sortIcon("displayName")}`;
    }
    get pointsSortLabel() {
        return `Points${this.sortIcon("points")}`;
    }
    get gamesSortLabel() {
        return `Games${this.sortIcon("games")}`;
    }
    get accuracySortLabel() {
        return `Accuracy${this.sortIcon("accuracy")}`;
    }
    get streakSortLabel() {
        return `Streak${this.sortIcon("currentStreak")}`;
    }
    get longestSortLabel() {
        return `Best${this.sortIcon("longestStreak")}`;
    }
    get achievementsSortLabel() {
        return `Badges${this.sortIcon("achievementCount")}`;
    }
    get lastPlayedSortLabel() {
        return `Last seen${this.sortIcon("lastPlayedAt")}`;
    }
    ...
    onWindowChange(e) {
        this.windowKey = e.detail.value;
        this.fetch();
    }
    onLimitChange(e) {
        this.rowLimit = Number(e.detail.value) || 25;
        this.fetch();
    }
    onSearch(e) {
    ...
    onSort(e) {
        ...
        if (this.sortBy === col) {
            ...
            // sensible defaults
            this.sortDir =
                col === "displayName" ? "asc" : col === "rank" ? "asc" : "desc";
        }
    }
    ...
    toast(message, variant) {
        this.dispatchEvent(
            new ShowToastEvent({ message, variant: variant || "info" })
        );
    }
    msg(e) {
