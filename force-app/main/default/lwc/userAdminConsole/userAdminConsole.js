...
export default class UserAdminConsole extends LightningElement {
    ...
    get isLoading() {
        return !(
            ...
            (this.wiredUsersResult.data || this.wiredUsersResult.error)
        );
    }

    get loadError() {
        return this.wiredUsersResult && this.wiredUsersResult.error
            ? (this.wiredUsersResult.error.body &&
                  this.wiredUsersResult.error.body.message) ||
    ...
    get summaryRow() {
        const s =
            (this.wiredSummaryResult && this.wiredSummaryResult.data) || {};
        return {
            players: s.players || 0,
            ...
            .catch((err) => {
                this.dispatchEvent(
                    new ShowToastEvent({
                        ...
                        message:
                            (err && err.body && err.body.message) || "unknown",
                        variant: "error"
                    })
                );
            });
