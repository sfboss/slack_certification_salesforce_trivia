...
export default class TournamentConsole extends LightningElement {
    ...
    wiredExams(result) {
        ...
        if (result.data) {
            this.examOptions = result.data.map((o) => ({
                label: o.label,
                value: o.id
            }));
        }
    ...
    get publicLink() {
        if (
            !this.detail ||
            !this.detail.tournament ||
            !this.detail.tournament.publicJoinToken
        )
            return "";
        const base =
    ...
    get overviewActive() {
        return this.detailTab === "overview";
    }
    get leaderboardActive() {
        return this.detailTab === "leaderboard";
    }
    get participantsActive() {
        return this.detailTab === "participants";
    }
    get settingsActive() {
        return this.detailTab === "settings";
    }
    get isEnrollSearchTab() {
        return this.enrollTab === "search";
    }
    get isEnrollBulkTab() {
        return this.enrollTab === "bulk";
    }
    ...
    get topThree() {
        ...
        return this.detail.participants
            ...
            .map((p) => ({
                ...p,
                podiumClass: "podium podium-" + p.rank,
                initials: this._initials(p.name)
            }));
    }
    ...
    get canStart() {
        return (
            this.detail &&
            this.detail.tournament &&
            this.detail.tournament.status === "Scheduled"
        );
    }
    get canComplete() {
        return (
            this.detail &&
            this.detail.tournament &&
            this.detail.tournament.status === "Active"
        );
    }
    get canCancel() {
        return (
            this.detail &&
            this.detail.tournament &&
            this.detail.tournament.status !== "Cancelled" &&
            this.detail.tournament.status !== "Complete"
        );
    }
    ...
    async refresh() {
        try {
            ...
            this.error = null;
            const promises = [];
            if (this._wiredTournamentsResult)
                promises.push(refreshApex(this._wiredTournamentsResult));
            if (this._wiredExamsResult)
                promises.push(refreshApex(this._wiredExamsResult));
    ...
    handleFormChange(e) {
        ...
        let v =
            e.target.type === "checkbox" ? e.target.checked : e.detail.value;
    ...
    async handleSaveTournament() {
        if (!this.form.name || !this.form.examId || !this.form.bracketType) {
            this._toast(
                "Missing",
                "Name, exam, and bracket type are required.",
                "warning"
            );
            return;
        }
    ...
    async handleStart() {
        if (
            !confirm(
                "Start this tournament? Public players will be able to play."
            )
        )
            return;
        try {
    ...
    async handleComplete() {
        if (!confirm("Complete this tournament and declare the winner?"))
            return;
        try {
            this.loading = true;
    ...
    handleCopyLink() {
        ...
        try {
            if (navigator && navigator.clipboard) {
                ...
                this._toast(
                    "Copied",
                    "Public join link copied to clipboard.",
                    "success"
                );
            } else {
            ...
            this._toast("Copy failed", this._msg(e), "warning");
        }
    }
    ...
    handleEnrollTab(e) {
        this.enrollTab = e.currentTarget.dataset.tab;
    }
    handleSearchInput(e) {
    ...
    handleBulkInput(e) {
        this.bulkEmails = e.target.value;
    }
    ...
    async _runSearch() {
        try {
            const rows = await searchPlayers({
                tournamentId: this.selectedId,
                query: this.searchQuery || ""
            });
            this.searchResults = rows;
        } catch (e) {
            this._toast("Search failed", this._msg(e), "error");
        }
    }
    ...
    async handleAddPlayer(e) {
        ...
        try {
            ...
            this._toast("Error", this._msg(e2), "error");
        } finally {
    ...
    async handleBulkSubmit() {
        const list = (this.bulkEmails || "")
            .split(/[\s,;]+/)
            .map((s) => s.trim())
            .filter(Boolean);
        if (list.length === 0) {
            this._toast("Empty", "Paste at least one email.", "warning");
            return;
        }
        try {
            this.enrolling = true;
            const r = await enrollByEmail({
                tournamentId: this.selectedId,
                emails: list
            });
            ...
            if (r.notFoundEmails && r.notFoundEmails.length > 0) {
                msg +=
                    " (" +
                    r.notFoundEmails.slice(0, 5).join(", ") +
                    (r.notFoundEmails.length > 5 ? "…" : "") +
                    ")";
            }
            this._toast(
                "Bulk enroll",
                msg,
                r.notFound > 0 ? "warning" : "success"
            );
            this.bulkEmails = "";
            await this._loadDetail(this.selectedId);
        } catch (e) {
