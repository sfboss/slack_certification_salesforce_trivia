...
export default class TournamentBuilder extends LightningElement {
    ...
    handleType(e) {
        this.bracketType = e.detail.value;
    }
    handleExam(e) {
        this.examId = e.detail.value;
    }
    handlePlayers(e) {
        this.playerIdsCsv = e.target.value;
    }

    async handleCreate() {
        if (!this.examId) {
            this.toast("Missing", "Select a certification exam.", "warning");
            return;
        }
        try {
    ...
    async handleBuild() {
        if (!this.tournamentId) {
            this.toast("Missing", "Create the tournament first.", "warning");
            return;
        }
        const ids = this.playerIdsCsv
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean);
        try {
            this.bracketJson = await buildBracket({
    ...
    toast(t, m, v) {
        this.dispatchEvent(
            new ShowToastEvent({ title: t, message: m, variant: v })
        );
    }
    msg(e) {
        return e && e.body && e.body.message ? e.body.message : e.message;
    }
}