...
export default class CertGameAdminHome extends LightningElement {
    ...
    async handleRefresh() {
        ...
        const dashboard = this.template.querySelector('c-cert-game-admin-dashboard');
    ...
    handleActionClick(event) {
        ...
        if (action === 'gen') {
            this.activeTab = 'gen';
            this.toast('Generation jobs', 'Open the Generation Jobs tab to manage live generation activity.', 'info');
    ...
    handleOverflowAction(event) {
        ...
        if (action === 'export') {
            this.toast('Export', 'Export actions are not wired in this console yet.', 'info');
            return;
        }

        this.toast('Settings', 'Settings remain managed in Salesforce setup and metadata.', 'info');
    }

    handleHealthAction(event) {
        this.questionBankFocus = event.detail.focusLabel;
        this.activeTab = 'bank';
        this.toast('Question Bank focus', `${event.detail.focusLabel} highlighted in Question Bank.`, 'warning');
    }

    handleDashboardNavigate(event) {
        this.activeTab = event.detail.tab;
        if (event.detail.message) {
            this.toast(event.detail.title || 'Updated', event.detail.message, event.detail.variant || 'info');
        }
    }

    get tabs() {
        const questionHealth = this.summaryData && this.summaryData.questionHealth ? this.summaryData.questionHealth : {};
        const recentGenerations = this.summaryData && this.summaryData.recentGenerations ? this.summaryData.recentGenerations : [];
        const totalPlayers = this.summaryData && this.summaryData.kpis ? this.summaryData.kpis.totalPlayers || 0 : 0;
        const reviewCount = questionHealth.draft || 0;
        const bankAttention = (questionHealth.brokenCitations || 0) + (questionHealth.lowQuality || 0);
        const generationAttention = recentGenerations.filter(job => job.status === 'Failed').length;
        ...
        return TAB_DEFS.map(tab => {
            ...
            if (tab.key === 'leaderboard') {
                ...
                attentionCount = generationAttention > 0 ? generationAttention : null;
            }
    ...
    get metadataItems() {
        ...
        return [
            { key: 'players', label: 'Players', value: this.formatNumber(kpis.totalPlayers || 0) },
            { key: 'published', label: 'Published Qs', value: this.formatNumber(questionHealth.published || 0) },
            { key: 'games', label: 'Active Games', value: this.formatNumber(kpis.gamesStarted7d || 0) },
            { key: 'cost', label: 'LLM Cost MTD', value: this.formatCurrency(usage.llmCostUsd) },
            { key: 'citations', label: 'Broken Citations', value: this.formatNumber(questionHealth.brokenCitations || 0) }
        ];
    }

    get tenantName() {
        return this.summaryData && this.summaryData.tenant ? this.summaryData.tenant.name : 'All Workspaces';
    }
    ...
    get pagePlan() {
        return this.summaryData && this.summaryData.tenant ? this.summaryData.tenant.plan : 'Org Overview';
    }

    get pageStatus() {
        return this.summaryData && this.summaryData.tenant ? this.summaryData.tenant.status : 'Active';
    }
