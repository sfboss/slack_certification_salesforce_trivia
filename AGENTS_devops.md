...
**Hard rule:** any workflow that _creates_ a scratch org MUST register a cleanup step that runs `if: always()` to delete it, unless the workflow's explicit purpose is to provision a long-lived org. This protects the 2-active-scratch-org Dev Hub limit.
